...
COLD_FIELD_ENABLED = True

# 间隔时间（秒），无弹幕超过该时长后播报一次冷场话术
COLD_FIELD_INTERVAL = 20

# 冷场话术列表（随机选择，建议10条以上以保证变化感）
# 仅当 TTS 空闲（不在回答弹幕问题）时才会播报
COLD_FIELD_PHRASES = [
...
# 同一用户欢迎冷却时间（秒），防止用户频繁进出导致刷屏
WELCOME_COOLDOWN = 10

# 欢迎语模板列表（随机选择，{user} 会被替换为用户名）
WELCOME_MESSAGES = [
    "欢迎 @{user} 进入直播间~",
    "@{user} 来啦！欢迎欢迎！",
    "欢迎新朋友 @{user}！有什么问题随时弹幕问我哦~",
    "@{user} 你好呀，点个关注不迷路！",
    "欢迎 @{user}！喜欢主播的可以支持一波~",
]
